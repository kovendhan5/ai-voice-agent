...
Production-ready version with proper configuration management
"""
...
try:
    ...
    print("⚠️ Configuration module not found, using defaults")
    class DefaultConfig:
        PORT = 8080
        HOST = '0.0.0.0'
        DEBUG = False
...
if hasattr(config, 'DISABLE_TRANSFORMERS') and config.DISABLE_TRANSFORMERS:
...
def index():
    ...
    try:
        # Look for the interface file in the same directory as app.py
        interface_path = os.path.join(os.path.dirname(__file__), 'voice_chat_interface.html')
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("🎭 ORPHEUS VOICE CHAT SERVER")
    print("="*60)
    print("🔥 Features:")
    ...
    print(f"\n🎯 Model: {getattr(config, 'ORPHEUS_MODEL_NAME', 'canopylabs/orpheus-tts-0.1-finetune-prod')}")
